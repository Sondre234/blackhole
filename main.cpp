...
void main() {
  ...
  if (panel > 0.5) {
    ...
    float activeLayer = 0.0;
    if (c < 0.18) activeLayer = 0.0;
    else if (c < 0.36) activeLayer = 1.0;
    else if (c < 0.54) activeLayer = 2.0;
    else if (c < 0.68) activeLayer = 3.0;
    else if (c < kSupernovaEnd) activeLayer = 4.0;
    else activeLayer = 5.0;

    for (int i = 0; i < 6; i++) {
      float o = float(i) * 0.018;
      float m = ringMask(vUV, center, baseR - o - 0.0175, baseR - o);
      float layerIsActive = 1.0 - step(0.5, abs(float(i) - activeLayer));
      float pulse = 0.75 + 0.25 * sin(40.0 * vUV.x + 30.0 * vUV.y + c * 30.0);
      vec3 lc = layerCols[i] * (1.0 + 0.45 * layerIsActive * pulse);
    ...
    for (int i = 0; i < 6; i++) {
      ...
      float layerIsActive = 1.0 - step(0.5, abs(float(i) - activeLayer));
      vec3 lc = layerCols[i] * (0.85 + 0.5 * layerIsActive);
      col = mix(col, lc, lm);
    }
  }
